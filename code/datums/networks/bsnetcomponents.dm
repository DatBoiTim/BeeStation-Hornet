...
/datum/interface //Parent Class for Interfaces. Despite identical fields, this is a non-functional interface due to inheritance reasons
...
*/
/datum/interface/proc/send_packet(destination, packFlag, packProtocol, packType, packData, signalTranMethod=1, range=-1)
...
/datum/interface/proc/recieve_radio_signal(datum/signal/S)
	var/datum/packet/P
...
/datum/interface/wired/send_packet(destination, packFlag, packProtocol, packType, packData, signalTranMethod=0, range=-1)
	if(src.powernet) //Is it plugged in?
		..(destination, packFlag, packProtocol, packType, packData, signalTranMethod, range)
	else
		return FALSE

/datum/interface/wired/recieve_radio_signal(datum/signal/S)
	if(src.powernet) //Is it plugged in?
		..(S)
	else
		return FALSE
